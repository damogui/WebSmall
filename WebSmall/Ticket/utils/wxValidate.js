	...
	__initDefaults() {
		this.defaults = {
			messages: {
				...
				taxNum: this.formatTpl('请输入有效的纳税人识别号'),
				totlelength: this.formatTpl('最多输入50个汉字或者100个字符')
			}
		}
	}
		...
		that.methods = {
			...
			taxNum(value) {
				...
				if(/^([a-zA-Z0-9]){15}$/.test(value)){
					...
					tax = true;
				}else if(/^([a-zA-Z0-9]){20}$/.test(value)){
					tax = true;
				}else{
					tax = false;
				}
				return that.optional(value) || tax;
			},
			/**
			 * 验证整数
			 */
			...
			totlelength(value, param) {
				...
				var num = checkLength(value)
				// console.log('数字字母总数：'+num,'param：'+param)
				return that.optional(value) || num <= param
			},
			/**
			 * 验证一个长度范围[min, max]
			 */
